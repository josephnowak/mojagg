"""Benchmark every width/unroll combination for NaN-aware sum.

Each measurement uses the same contiguous floating-point reduction and differs
...
def vectorized_variant[width: Int, unroll_factor: Int](
    ...
    def step[vector_width: Int](i: Int, evl: Int) {
        ...
        block = isnan(block).select(zero, block)
        acc += block

    vectorize[width, unroll_factor=unroll_factor](length, step)
    output[unsafe_offset=0] = acc.reduce_add()
...
def results_match(left_address: Int, right_address: Int) -> Bool:
    ...
    var difference = left[unsafe_offset=0] - right[unsafe_offset=0]
    if difference < 0:
        difference = -difference
    var tolerance = right[unsafe_offset=0] * 1.0e-12 + 1.0e-6
...
def main() raises:
    ...
    var reference_storage = alloc(Layout[Float64](count=1))
    var output_storage = alloc(Layout[Float64](count=1))
