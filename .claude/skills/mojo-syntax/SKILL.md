...
(`var ref = ...` → `"unexpected token in expression"`). The convention
words `imm`, `read`, `mut`, `out`, `deinit` are soft keywords: fine as local
variable or `[...]` parameter names, but invalid as argument names
