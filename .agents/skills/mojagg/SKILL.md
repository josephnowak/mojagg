...
1. **Zero-copy boundary**: binding takes `arr.ctypes.data` → `Pointer` → `Span`/`NDView`. Validate dtype/ndim ONCE; then never touch Python until return. Never call `ascontiguousarray` silently — reject instead.
...
3. **SIMD**: vector accumulators in-register; masked NaN handling via `v != v` unordered compare; horizontal reduce once at the end. f32 must genuinely process 2× f64 lanes. **Prefer `std.algorithm.vectorize[W](size, fn)` over hand-written SIMD main+tail loops** — same codegen, less code, the idiom NuMojo validates. Hand-rolled loops only when vectorize can't express the pattern (e.g. multi-accumulator interleavings).
4. **Parallel reductions**: per-worker accumulator slices, **cache-line padded (64 B)** to avoid false sharing; single-thread tree merge. Parallel via `max.algorithm.parallelize` with `def` closures — **`@parameter` is deprecated (Mojo closures V1)**.
...
6. **Strided fallback**: contiguous fast path and strided path are separate instantiations; the hot loop never branches on stride. The per-slice `stride==1` branch lives in the DRIVER, not the kernel.
7. Every optimization must show up in the benchmark matrix or be reverted.

### Mojo 1.0 idioms (verified in this repo — follow these)

- `Pointer[mut=True, T, MutAnyOrigin](unsafe_from_address=Int(py=arr.ctypes.data))`; `ptr.unsafe_offset(i)` (the `+` operator is deprecated); `ptr[unsafe_offset=i]`.
- `parallelize`: `from max.algorithm import parallelize`; V1 closure `def body(i: Int) {mut acc}: ...` passed as a RUNTIME arg — `parallelize(body, n)` (the `parallelize[FuncType: def(Int) -> None](func, ...)` overload). Never `@parameter`.
- Struct fields cannot hold `Pointer[_, MutAnyOrigin]` (compile error) — store raw `Int` addresses + `ptr()` accessors (NDView/ReducePlan pattern).
- `Array[Int, N]` = Mojo 1.0's fixed-length inline array (`fill=` ctor, len == N, no append/push). Use it (as `DimArray`) for rank metadata; the comptime capacity bound (`MAX_NDIM`) is defined once in `core/ndview.mojo`.
- `Bool` ≠ `Scalar[DType.bool]` — bool-out kernels return `Scalar[DType.bool]` so drivers stay DType-typed.
- Static struct methods are `thin` fns — driver hook params are declared `def (...) thin -> T` (no `capturing`).
- `Self.<param>` for comptime struct params inside bodies; `InlineArray[T, N](fill=...)` and explicit `.copy()` when assigning (not ImplicitlyCopyable).
- **Vectorized NaN compares miscompile in Mojo 1.0** on memory-loaded SIMD (`v.ne(v)` folds wrong); use per-lane `std.math.isnan(v[k])` until fixed (mojo-watch item). Accumulating `eq().select()` patterns pass parity tests but verify per op.
- `out` is a reserved keyword — never a parameter name.
