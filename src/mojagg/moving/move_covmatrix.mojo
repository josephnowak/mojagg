"""NaN-aware trailing moving covariance matrices."""
...
from mojagg.moving.moving_matrix_helpers import (
    ...
    _leading_rows_shift,
    _move_cov_pair,
    _move_cov_pair_full_final,
)


struct MoveCovMatrixKernel[dtype: DType](GUFuncKernel, ImplicitlyCopyable):
    """SIMD-driver-compatible ``(m,n) -> (m,n,n)`` rolling covariance."""
    ...
    def __call__(mut self, tensors: Self.Signature):
        ...
        for i in range(self.n_vars):
            ...
            for j in range(i, self.n_vars):
                ...
                if leading_rows == self.n_obs:
                    _move_cov_pair_full_final[Self.dtype](
                        source,
                        destination,
                        self.n_obs,
                        self.n_vars,
