...
def _move_exp_nancov[
    ...
    for i in range(len(values_a)):
        ...
        if weight >= min_weight and sum_weight != 0.0:
            ...
            if bias > 0.0:
                output = (cov_biased / bias).cast[dtype]()
...
struct MoveExpNanCovKernel[dtype: DType](GUFuncKernel, ImplicitlyCopyable):
    ...
    def __call__(mut self, tensors: Self.Signature):
        ...
        _move_exp_nancov[Self.dtype, False](
            input_a.read_span(),
            input_b.read_span(),
            alpha.read_span(),
            output.write_span(),
...
struct MoveExpNanCovScalarKernel[dtype: DType](
    GUFuncKernel, ImplicitlyCopyable
):
    """``(n), (n), () -> (n)`` moving covariance for scalar alpha."""
    ...
    def __call__(mut self, tensors: Self.Signature):
        var input_a, input_b, alpha, output = tensors
        _move_exp_nancov[Self.dtype, True](
            input_a.read_span(),
            input_b.read_span(),
            alpha.read_span(),
            output.write_span(),
