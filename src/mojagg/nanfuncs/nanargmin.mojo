"""NaN-aware first-occurrence argmin reduction."""

from std.collections import Span
from std.math import isnan, iota
from std.sys.info import simd_width_of

from mojagg.core.numeric import pos_inf_or_max
from mojagg.core.reduce1d import Reduction1D


@fieldwise_init
struct ArgState[dtype: DType](Copyable):
    var value: Scalar[Self.dtype]
    var index: Int64


struct NanArgMin[dtype: DType](Reduction1D):
    ...
    def identity() -> Self.State:
        return Self.State(pos_inf_or_max[Self.dtype](), -1)
    ...
    def contig(data: Span[Scalar[Self.dtype], _]) -> Self.State:
        ...
        var global_values = SIMD[Self.dtype, W](pos_inf_or_max[Self.dtype]())
        ...
        while i < vec_n:
            ...
            comptime if Self.dtype.is_floating_point():
                ...
                clean_values = is_nan.select(
                    SIMD[Self.dtype, W](pos_inf_or_max[Self.dtype]()),
                    curr_values,
                )
                var is_valid = ~is_nan
                take = clean_values.lt(global_values) | (
                    global_indices.eq(-1) & is_valid
                )
            else:
                clean_values = curr_values
                take = clean_values.lt(global_values) | global_indices.eq(-1)
        ...
        var best_val = pos_inf_or_max[Self.dtype]()
        ...
        if has_valid:
            ...
            best_val = valid_values.reduce_min()
